...
public class RSA {
    ...
    public String encryptMessage(String plaintext) {
        String result = "";
        for(int j = 0; j < plaintext.length() % 5; j++)
            plaintext += " ";
        
        for(int i = 0; i < plaintext.length(); i+=5) 
        {
            String text = plaintext.substring(i, 5+i);
            BigInteger r = NumberHelpers.recursiveModularExponentiation(AlphabetConversion.stringToNumber(text), e, n);
            result += r.toString() + "\n";
        }
    ...
    public String decrypt(String cipherText) {
        ...
        for (String s : numbers) {
            ...
            result += AlphabetConversion.numberToString(NumberHelpers.recursiveModularExponentiation(number, d, n));
        }
